...
const MOBILE_THRUST_START_DISTANCE = 0.50;
const MOBILE_THRUST_FULL_DISTANCE = 0.95;
          ...
          if (isTouchDevice) {
            ...
            if (hasActiveTouchJoystick) {
              ...
              if (absDiff <= Math.max(MOBILE_TARGET_SNAP_ANGLE, turnStep)) {
                ...
                ship.angle += Math.sign(angleDiff) * turnStep;
              }

              const distanceThrust = Math.min(1, Math.max(0, 
                (joystick.distance - MOBILE_THRUST_START_DISTANCE) /
                (MOBILE_THRUST_FULL_DISTANCE - MOBILE_THRUST_START_DISTANCE)
              ));

              const alignmentFactor = Math.min(1, Math.max(0, 
                1 - (absDiff - MOBILE_FULL_THRUST_ALIGNMENT_ANGLE) / 
                    (MOBILE_THRUST_CUTOFF_ANGLE - MOBILE_FULL_THRUST_ALIGNMENT_ANGLE)
              ));

              mobileThrustStrength = distanceThrust * alignmentFactor;
              if (mobileThrustStrength > 0.01) {
